...
const LetterOfSuspension = ({ studentDataToPass, violationData ,alertMessageFunction }) => {
    ...
    function getSanctionByViolation(student, rules, selectedCode) {
        ...
        let selectedViolation = student.violations.find(v => v.code.trim() === selectedCode.trim());
        if (!selectedViolation) {
            console.error(`Violation with code ${selectedCode} not found in student data.`);
        ...
        let selectedRule = rules.find(r => r.violations.some(vr => vr.code.trim() === selectedCode.trim()));
        if (!selectedRule) {
            console.error(`No rule found for violation code ${selectedCode}.`);
            return { sanction: "No rule found for this violation.", description: selectedViolation.description };
        }
    
        console.log("Selected Rule:", selectedRule);
    
        // Get the correct description from the rules
        let correctViolation = selectedRule.violations.find(v => v.code.trim() === selectedCode.trim());
        if (correctViolation) {
            selectedViolation.description = correctViolation.description; // Fix the description
        }
    
        console.log("Updated Violation Description:", selectedViolation.description);
        ...
        student.violations.forEach(v => {
            let rule = rules.find(r => r.violations.some(vr => vr.code.trim() === v.code.trim()));
            if (rule) {
        ...
        let offenseCount = (selectedRule.category === "minor") ? minorCount : majorCount;
        ...
        let sanction = selectedRule.sanctions[offenseLevel] || "No specific sanction found";
    ...
    useEffect(() => {
        if (studentData.length > 0 && studentData[0].violations.length > 0) {
            ...
            const sanctionDetails = getSanctionByViolation(firstStudent, violationData, defaultViolationCode);
            ...
            setFormData((prev) => ({
                ...
                college: "Batangas State University",
                year: firstStudent.year_and_department ? firstStudent.year_and_department.split(" - ")[0] : "",
                campus: "Batangas State University - Nasugbu Campus",
                report_date: new Date().toISOString().slice(0, 10),
                report_time: new Date().toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit" }),
            }));
    ...
    const handleViolationChange = (e) => {
        const newViolationCode = e.target.value;
        setSelectedViolation(newViolationCode);
    
        const sanctionDetails = getSanctionByViolation(studentDataToPass, violationData, newViolationCode);
    
        setFormData((prev) => ({
          ...prev,
          violation_code: newViolationCode,
          violation_description: sanctionDetails.description || "",
          sanction: sanctionDetails.sanction || "",
        }));
    };
    const handleSubmit = (e) => {
        ...
        axios.post("form/letter_of_suspension", formData)
        .then((response) => {
        ...
        .catch((error) => {
            ...
            alertMessageFunction(true, "An error occurred while submitting the form.", "Error");
        })
        .finally(() => {
            setIsLoading(false);
    ...
    return (
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-4 w-full mt-4">
            ...
            <InputLabel id="demo-simple-select-helper-label">Violation Code</InputLabel>
            <Select
              ...
              onChange={(e) => {
                ...
                const sanctionDetails = getSanctionByViolation(studentData[0], violationData, e.target.value);
                setFormData((prev) => ({
                  ...prev,
              ...
              >
               {studentData.length > 0 && studentData[0].violations.map((violation, index) => (
            ...
            typeof formData[key] === "object" ? (
                ...
                const isDateField = subKey.includes("date") || subKey.includes("suspension");
                const dateValue = formData[key][subKey] ? new Date(formData[key][subKey]) : null;
                ...
                return (
                    ...
                    <TextField
                        ...
                        value={isDateField && dateValue instanceof Date && !isNaN(dateValue)
                ...
                <TextField
                    ...
                    name={key}
                    type={key.includes("date") || key.includes("suspension") ? "datetime-local" : "text"}
                    value={(key.includes("date") || key.includes("suspension")) &&
                    formData[key] &&
            ...
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.3, delay: Object.keys(formData).length * 0.1 }}
            >
              <FormControl fullWidth>
