...
const NoticeCaseDismissal = ({ studentDataToPass, alertMessageFunction }) => {
    ...
    if (name === "is_without_merit") {
      setFormData((prev) => ({
        ...
        reason_for_dismissal: {
          ...prev.reason_for_dismissal,
          is_without_merit: checked,
        },
      }));
    } else if (name === "others") {
      setFormData((prev) => ({
  ...
  const handleSubmit = (e) => {
    ...
    axios
      .post("/form/notice_case_dismissal", formData)
      .then((response) => {
        if (response.data.status === "success") {
          alertMessageFunction(
            true,
            "Notice of Case Dismissal form submitted successfully.",
            "Success"
          );
        } else {
          alertMessageFunction(
            true,
            "Notice of Case Dismissal form failed to submit.",
            "Error"
          );
        }
      })
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4 my-4">
        {Object.keys(formData)
          .filter((key) => key !== "coordinator_discipline_head")
          .map((key, index) => {
