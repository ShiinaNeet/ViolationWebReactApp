...
const CallSlipForm = ({ studentDataToPass, alertMessageFunction }) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post("/form/callslip", formData)
    .then((response) => {
      if(response.data.status === "success") {
        alertMessageFunction(true, "Call Slip Form submitted successfully.", "Success");
      }
      else {
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4 w-full mt-4">
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.3, delay: (Object.keys(formData).length + 1) * 0.1 }}
        >
          <Button type="submit" variant="outlined" color="error" fullWidth disabled={isLoading}>
